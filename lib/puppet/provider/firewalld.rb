...
class Puppet::Provider::Firewalld < Puppet::Provider
  ...
  # v3.0.0
  def self.execute_firewall_cmd(args, zone=nil, perm=true)
    cmd_args = []
  ...
  def execute_firewall_cmd(args, zone=@resource[:zone], perm=true)
    self.class.execute_firewall_cmd(args, zone, perm)
  end

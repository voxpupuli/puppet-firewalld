...
describe provider_class do
  ...
  before do
    provider.class.stubs(:execute_firewall_cmd).with(['--get-ipsets'], nil, false).returns('white black')
    ...
    provider.class.stubs(:execute_firewall_cmd).with(['--info-ipset=white'], nil, false).returns('white
  type: hash:ip
  options: maxelem=200 family=inet6
  entries:')
    provider.class.stubs(:execute_firewall_cmd).with(['--info-ipset=black'], nil, false).returns('black
  type: hash:ip
  options: maxelem=400 family=inet hashsize=2048
  entries:')
    provider.class.stubs(:execute_firewall_cmd).with(['--ipset=white', '--get-entries'], nil, false).returns('')
  end
